...
import { addWebhook, deleteWebhook, updateWebhook, getServices } from '../api';
...
export const WebhookManager: React.FC<WebhookManagerProps> = ({ serviceName }) => {
  ...
  const handleAddWebhook = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!token || !serviceId || !newUrl || selectedLevels.length === 0) {
      if (selectedLevels.length === 0) alert("Please select at least one log level.");
      return;
    }
    try {
      const nw = await addWebhook(token, serviceId, { 
        url: newUrl, 
  ...
  const handleUpdateWebhook = async (webhookId: string) => {
    if (!token || !serviceId || !editUrl || editLevels.length === 0) return;
    try {
      await updateWebhook(token, serviceId, webhookId, { 
        url: editUrl, 
        levels: editLevels 
      });
  ...
  const handleDeleteWebhook = async (webhookId: string) => {
    ...
    try {
      await deleteWebhook(token, serviceId, webhookId);
    ...
    <Card className="border-muted shadow-sm">
      <CardHeader>
        <div className="flex items-center gap-3">
          ...
          <div>
            <CardTitle>Alerting Webhooks</CardTitle>
            <CardDescription>Get notified on Slack/Discord when specific logs occur.</CardDescription>
          </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-6">
          ...
          <form onSubmit={handleAddWebhook} className="space-y-4 p-4 rounded-2xl bg-muted/30 border border-muted/50 animate-in fade-in slide-in-from-top-2 duration-300">
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                />
                <Button type="submit" size="sm" className="shrink-0 h-9 font-bold">
        ...
        <div className="space-y-3">
          <h3 className="text-[10px] font-black uppercase text-muted-foreground tracking-widest ml-1">Active Destinatons</h3>
          {webhooks.length === 0 && (
            ...
            <div key={webhook.id}>
              {editingId === webhook.id ? (
                <div className="space-y-4 p-4 rounded-2xl bg-orange-500/[0.03] border-2 border-orange-500/20 animate-in zoom-in-95 duration-200">
                   <div className="space-y-2">
                      <label className="text-[9px] font-black uppercase text-orange-600 tracking-widest">Update URL</label>
                      <Input 
                        value={editUrl}
                        onChange={(e) => setEditUrl(e.target.value)}
                        className="text-xs h-9 bg-background border-orange-200 focus-visible:ring-orange-500"
                      />
                   </div>
                   <div className="space-y-2">
                      <label className="text-[9px] font-black uppercase text-orange-600 tracking-widest">Update Levels</label>
                      <div className="flex flex-wrap gap-2">
                        {AVAILABLE_LEVELS.map(level => (
                          <button
                            key={level}
                            ...
                            className={`px-3 py-0.5 rounded-full text-[9px] font-bold transition-all border ${
                   ...
                   <div className="flex gap-2 justify-end pt-2">
                      ...
                      <Button size="sm" onClick={() => handleUpdateWebhook(webhook.id)} className="h-8 text-xs bg-green-600 hover:bg-green-700 font-bold">
                        <Check className="w-3.5 h-3.5 mr-1" /> Save Changes
                      </Button>
                   </div>
                </div>
              ) : (
                <div className="flex items-center justify-between p-3 rounded-xl border bg-card group hover:border-orange-500/30 transition-colors">
                  <div className="flex flex-col gap-1 overflow-hidden">
                    <span className="text-[10px] font-mono text-muted-foreground truncate max-w-[200px] md:max-w-md">{webhook.url}</span>
                    <div className="flex gap-1">
                      {webhook.levels.map((l: string) => (
                        <Badge key={l} variant="outline" className="text-[8px] py-0 px-1 border-orange-500/30 text-orange-600 bg-orange-500/5 uppercase font-black">
                          {l}
                        </Badge>
                      ))}
                    </div>
        ...
        <div className="p-4 rounded-xl bg-blue-500/[0.03] border border-blue-500/10 flex gap-3 items-start">
          <AlertTriangle className="w-4 h-4 text-blue-600 shrink-0 mt-0.5" />
          <div className="space-y-1">
             <p className="text-[11px] font-bold text-blue-700">Configuration Info</p>
